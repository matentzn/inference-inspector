	...
	implements MouseListener, ActionListener, TableColumnModelListener, PropertyChangeListener
{
	...
	private Set<Integer> hiddenColumns = new HashSet<Integer>();
	...
	public TableColumn getColumn(String s) {
		return tcm.getColumn(tcm.getColumnIndex(s));
	}
...
//
    public void columnAdded(TableColumnModelEvent e)
    {
    	...
    	//System.out.println("Workaround: allColumns.contains() not working "+column.getHeaderValue());
    	boolean contains = false;
    	for(TableColumn tc:allColumns) {
    		if(tc.getHeaderValue().equals(column.getHeaderValue())) {
    			contains = true;
    			break;
    		}
    	}
