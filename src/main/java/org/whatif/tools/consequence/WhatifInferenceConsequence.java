...
public class WhatifInferenceConsequence implements WhatifConsequence {

	final OWLAxiom axiom;
	final Set<AxiomPattern> matchingpatterns;
	
	
	public WhatifInferenceConsequence(OWLAxiom ax, Set<AxiomPattern> matchingpatterns) {
		this.axiom = ax;
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof WhatifInferenceConsequence)) {
			return false;
		}
		WhatifInferenceConsequence other = (WhatifInferenceConsequence) obj;
		if (axiom == null) {
			if (other.axiom != null) {
				return false;
			}
	...
	public String getConsequenceType() {
		String o = getHighestPriorityAxiomPattern().toString();
		String s = getOWLAxiomType().getName();
		if(!o.equals("Other")) {
			s+=" ("+getHighestPriorityAxiomPattern().toString()+")";
		}
		return s;
	}
	...
	public AxiomPattern getHighestPriorityAxiomPattern() {
		...
		return at_out==null?AxiomPatternFactory.getDefaultPattern():at_out;
	}
